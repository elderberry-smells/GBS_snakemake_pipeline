    ...
    with open(fastq) as f1, open(fastq2) as f2:
        ...
        for r1, r2 in zip(f1, f2):
            ...
            if len(lines1) == 4:
                ...
                index_name = parse_index(record, barcodes)
                barcodes[index_name][1] += 1

                # read 1 sliced data
                sequence = str(record['sequence'])  # cut off the index, leaving just TCGA at start
                quality = str(record['quality'])  # cut off the same amount of quality scores

                # read 2 sliced data
                sequence2 = str(record2['sequence'])  # cut off the index, leaving just TCGA at start
                quality2 = str(record2['quality'])  # cut off the same amount of quality scores
